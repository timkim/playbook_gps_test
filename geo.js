...
/**
 * This tests if our wifi connection can actually hit the net 
 */		
function testWifi(win, fail){
    ...
    testXHR.open("GET","http://www.google.com", false);
    testXHR.send();
}	

/**
 * The function that actually calls the geolocation stuff
 */		    
function doGeo(params){
...
/**
 * Win callback to tell us that wifi is working
 */	
function wifiSuccess(params){
    displayOutput("Wifi available - let's see how long it takes to get a position");
    doGeo(params);    
}

/**
 * Fail callback to tell us that wifi isn't working 
 */	
...
function getPosition(params)
{
	try
	{
		...
		// See if wifi actually works and alert the user about it's status
		// then call geolocation regardless of wifi connectivity
        testWifi(function(params){wifiSuccess(params)}, function(params){wifiFail(params)});
